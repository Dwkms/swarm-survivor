...
현재 적이 화면 밖 **원주 전체에 균등 랜덤**으로 나옵니다. 플레이어 이동속도 5, 슬라임 2.0으로
2.5배 빠른데도 **사방에서 동시에 오면 포위되어 도망칠 수 없습니다.**

**원인은 마릿수가 아니라 분포입니다.** 마릿수를 줄여도 사방에 뿌리는 한 똑같이 포위되고,
스폰 수를 바꾸면 성능 측정 조건까지 다시 잡아야 합니다.

**마릿수를 유지한 채 각도만 제한하고 그 섹터를 회전시킵니다.**
`EnemySpawner.GetRandomPointOnCircle()`의 `Random.Range(0, 2π)`를
`Random.Range(sectorStart, sectorStart + sectorWidth)`로 바꾸고, `sectorStart`를 몇 초마다
돌리면 됩니다. 항상 비어 있는 방향이 존재하되 한 방향으로만 계속 도망칠 수는 없게 됩니다.
